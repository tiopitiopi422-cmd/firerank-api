...
if (!secret) {
  ...
  fetch(`${baseUrl}/api/internal/order-reminders`, {
    method: 'POST',
    headers: {
    ...
    .then(async (response) => {
      ...
      console.log(`FIRERANK_ORDER_REMINDERS=SUCCESS ${text.slice(0, 600)}`);
    })
    .catch((error) => {
      console.error(`FIRERANK_ORDER_REMINDERS=ERROR ${error.message}`);
      process.exitCode = 1;
    });
}
